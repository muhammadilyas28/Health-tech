...
function populateDetails() {
    ...
    // Create a cursor to iterate over records
    const request = objectStore.openCursor(null, 'prev'); // 'prev' orders the records in reverse

    request.onsuccess = (event) => {
        const cursor = event.target.result;

        if (cursor) {
            // Get the last record (cursor points to the last record in reverse order)
            const lastAppointment = cursor.value;
...
function downloadReceipt() {
    ...
    const titleElement = receiptContent.querySelector('.font-bold');
    ...
    const title = titleElement.textContent || 'Receipt';
    let y = 20;
    ...
    details.forEach(detail => {
        ...
        if (strongElement && spanElement) {
            const strong = strongElement.textContent || '';
            const span = spanElement.textContent || '';
            doc.text(`${strong} ${span}`, 10, y);
        } else {
            console.warn('Expected elements not found within detail:', detail);
        }
    });
